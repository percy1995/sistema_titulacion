...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.examenSp.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.examen-sps.update", [$examenSp->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('programaacademico') ? 'is-invalid' : '' }}" name="programaacademico_id" id="programaacademico_id" required>
                    @foreach($programaacademicos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('programaacademico_id') ? old('programaacademico_id') : $examenSp->programaacademico->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('programamodular') ? 'is-invalid' : '' }}" name="programamodular_id" id="programamodular_id">
                    @foreach($programamodulars as $id => $entry)
                        <option value="{{ $id }}" {{ (old('programamodular_id') ? old('programamodular_id') : $examenSp->programamodular->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('titulo') ? 'is-invalid' : '' }}" name="titulo" id="titulo">{{ old('titulo', $examenSp->titulo) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha') ? 'is-invalid' : '' }}" type="text" name="fecha" id="fecha" value="{{ old('fecha', $examenSp->fecha) }}" required>
                @if($errors->has('fecha'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fecha') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('horainicio') ? 'is-invalid' : '' }}" type="text" name="horainicio" id="horainicio" value="{{ old('horainicio', $examenSp->horainicio) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('horafin') ? 'is-invalid' : '' }}" type="text" name="horafin" id="horafin" value="{{ old('horafin', $examenSp->horafin) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nota') ? 'is-invalid' : '' }}" type="text" name="nota" id="nota" value="{{ old('nota', $examenSp->nota) }}">
                @if($errors->has('nota'))
                    <div class="invalid-feedback">
                        {{ $errors->first('nota') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('docente') ? 'is-invalid' : '' }}" name="docente_id" id="docente_id">
                    @foreach($docentes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('docente_id') ? old('docente_id') : $examenSp->docente->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
