...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.alumnos.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('traplipro') ? 'is-invalid' : '' }}" name="traplipro_id" id="traplipro_id">
                    @foreach($traplipros as $id => $entry)
                        <option value="{{ $id }}" {{ old('traplipro_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('correo') ? 'is-invalid' : '' }}" type="text" name="correo" id="correo" value="{{ old('correo', '') }}" required>
