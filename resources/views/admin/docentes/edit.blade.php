    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.docentes.update", [$docente->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('correoinstitucional') ? 'is-invalid' : '' }}" type="text" name="correoinstitucional" id="correoinstitucional" value="{{ old('correoinstitucional', $docente->correoinstitucional) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tipo') ? 'is-invalid' : '' }}" type="text" name="tipo" id="tipo" value="{{ old('tipo', $docente->tipo) }}" required>
                @if($errors->has('tipo'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('persona') ? 'is-invalid' : '' }}" name="persona_id" id="persona_id">
                    @foreach($personas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('persona_id') ? old('persona_id') : $docente->persona->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('programa') ? 'is-invalid' : '' }}" name="programa_id" id="programa_id">
                    @foreach($programas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('programa_id') ? old('programa_id') : $docente->programa->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
