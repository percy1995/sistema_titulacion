...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.programaModular.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.programa-modulars.update", [$programaModular->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('programaacademico') ? 'is-invalid' : '' }}" name="programaacademico_id" id="programaacademico_id">
                    @foreach($programaacademicos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('programaacademico_id') ? old('programaacademico_id') : $programaModular->programaacademico->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombreprograma') ? 'is-invalid' : '' }}" type="text" name="nombreprograma" id="nombreprograma" value="{{ old('nombreprograma', $programaModular->nombreprograma) }}" required>
